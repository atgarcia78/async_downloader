...
class AsyncHTTPDownloader:
    ...
    def init(self):
        try:
            ...
            with self.sem:  # type: ignore
                ...
                if not _mult_ranges:
                    logger.info(f"{self.premsg}: server cant handle ranges")
                    self.n_parts = 1
                    self._NUM_WORKERS = 1
    ...
    async def fetch(self, i):
        ...
        try:
                        ...
                        async with (
                            ...
                            if res.status_code >= 400:
                                ...
                                if (len(self.parts[part - 1]["headers"]) == 1) and (
                                    ...
                                    self.parts[part - 1]["headersize"] = int_or_none(
            ...
            await client.aclose()
            logger.debug(f"{self.premsg}[worker-{i}] says bye")
            self.count -= 1
    ...
    def print_hookup(self):
        ...
        if self.status == "done":
            ...
            _progress_str = f"{_temp_size * 100:5.2f}%" if self.filesize else "-----"
            return (
