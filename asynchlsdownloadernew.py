...
class AsyncHLSDownloader:
    ...
    async def _download_frag(self, index: int, msg: str, nco: int):
        ...
        async def _handle_iter(ctx, data):
            ...
            if data:
                ...
                if len(ctx.data) >= 512 * self._CHUNK_SIZE:
                    try:
                        await ctx.stream.write(ctx.data)
                        ctx.data = b''
                    except Exception as e:
                        logger.error(f"{_premsg}[stream_write]: Error: {repr(e)}")
                ...
                if ctx.data:
                    try:
                        ...
                        logger.error(f"{_premsg}[stream_write][last]: Error: {repr(e)}")
    ...
    async def upt_status(self):
        ...
        while not self._vid_dl.end_tasks.is_set():
            if _timer.has_elapsed(seconds=CONF_INTERVAL_GUI) and not self.check_any_event_is_set():
                await _upt()
