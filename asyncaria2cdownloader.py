...
class AsyncARIA2CDownloader:
    ...
    def __init__(self, port, args, video_dict, vid_dl):
        ...
        def getter(name):
            value, key_text = getter_basic_config_extr(name, AsyncARIA2CDownloader._CONFIG) or (None, None)
    ...
    async def add_uris(self, uris: list[str]) -> Union[aria2p.Download, None]:
        async with self._decor:
            return await self._acall(AsyncARIA2CDownloader.aria2_API.add_uris, uris, options=self.opts)
    ...
    async def update_uri(self):
        ...
        if self._mode == "noproxy":
            ...
            if self._mode == "simple":
                ...
                try:
                    async with myYTDL(
                        params=self.ytdl.params, silent=True, proxy=self._proxy, executor=self.ex_dl
                    ) as proxy_ytdl:
                    ...
                    _proxy_info_url = cast(
                        str, try_get(traverse_obj(proxy_info, ("url")), lambda x: unquote(x) if x else None)
                    )
                ...
                if self.filesize:
                    ...
                    _gr = min(_gr, CONF_PROXIES_N_GR_VIDEO)
                else:
    ...
    async def check_speed(self):
        ...
        try:
            while True:
                _res = await async_waitfortasks(
                    self._qspeed.get(), events=self.vid_dl.stop_event, background_tasks=self.background_tasks
                )
                if "event" in _res:
    ...
    async def event_handle(self):
        ...
        if self.vid_dl.pause_event.is_set():
            ...
            if _event := [
                _ev.name for _ev in (self.vid_dl.reset_event, self.vid_dl.stop_event) if _ev.is_set()
            ]:
                _res = {"event": _event}
                self.progress_timer.reset()
    ...
    async def fetch_async(self):
        ...
        # def _print_el(element: tuple):
        #     _secs = element[0].second + element[0].microsecond / 1000000
        #     _str0 = f'{element[0].strftime("%H:%M:")}{_secs:06.3f}'
        #     if isinstance(element[1], (str, list, tuple)):
        #         _str1 = element[1]
        #     else:
        #         _str1 = f"['status': {element[1].status}, 'speed': {element[1].download_speed}]"
    ...
    def print_hookup(self):
        ...
        if self.status == "done":
            ...
            msg = (
                f'[ARIA2C][{self.info_dict["format_id"]}]: HOST[{self._host.split(".")[0]}] Waiting '
                + f'{naturalsize(self.down_size, format_=".2f")} '
                + f'[{naturalsize(self.filesize, format_=".2f") if self.filesize else "NA"}]\n'
            )
        elif self.status == "error":
            msg = (
                f'[ARIA2C][{self.info_dict["format_id"]}]: HOST[{self._host.split(".")[0]}] ERROR '
                + f'{naturalsize(self.down_size, format_=".2f")} '
                + f'[{naturalsize(self.filesize, format_=".2f") if self.filesize else "NA"}]\n'
            )
        elif self.status == "stop":
            msg = (
                f'[ARIA2C][{self.info_dict["format_id"]}]: HOST[{self._host.split(".")[0]}] STOPPED '
                + f'{naturalsize(self.down_size, format_=".2f")} '
                + f'[{naturalsize(self.filesize, format_=".2f") if self.filesize else "NA"}]\n'
            )
            ...
            if (
                ...
                _speed_temp = naturalsize(_temp.download_speed, binary=True)
                ...
                msg = (
                    f'[ARIA2C][{self.info_dict["format_id"]}]: HOST'
                ...
                msg = (
                    f'[ARIA2C][{self.info_dict["format_id"]}]: '
            ...
            if self.filename.exists():
                ...
                _size = 0
            msg = (
                f'[ARIA2C][{self.info_dict["format_id"]}]: Ensambling '
