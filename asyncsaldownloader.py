...
from yt_dlp.utils import shell_quote, parse_filesize, smuggle_url
...
class AsyncSALDownloader:
    ...
    async def async_terminate(self, pid, msg=None):
        ...
        if msg:
            premsg += f" {msg}"
        logger.debug(
            f"[{self.info_dict['id']}][{self.info_dict['title']}]{premsg} terminate proc {self._proc[pid]}"
        )
    ...
    async def event_handle(self, pid) -> dict:
        ...
        if self.vid_dl.pause_event.is_set():
            ...
            logger.debug(
                f"[{self.info_dict['id']}][{self.info_dict['title']}][ev_handle] wait for event: {_print_ev}"
            )
            ...
            if "readycheck" in _event:
                ...
                if results[1] == 0:
                    ...
                    if results[1] == 1 and "exists, quiting..." in results[0]:
                        ...
                        await asyncio.sleep(0)
                        _msgerr = (
                            try_get(re.search(r"\[fatal\]\s*.*", results[0]), lambda x: x.group())
                            or results[0]
                        )
                    ...
                    if not re.match(r"\d+", _line):
                        ...
                        if _prog_info := re.search(self.progress_pattern, _buffer_prog):
                            ...
                            if _dl_size:
                                ...
                                _status.update({"download_bytes": _dl_size, "speed_meter": _speed_meter})
                    ...
                    try:
                        while True:
                            ...
                            if error := (_res.get("error") or ("error" if self.status == "error" else None)):
                                logger.error(
                                    f"[{self.info_dict['id']}][{self.info_dict['title']}][fetch_async] {error}"
                                )
                                return
                        ...
                        logger.exception(
                            f"[{self.info_dict['id']}][{self.info_dict['title']}] inner error {repr(e)}"
                        )
                        self.status = "error"
                        return
                    finally:
    ...
    def print_hookup(self):
        ...
        try:
            if self.status == "done":
                ...
                msg = (
                    f'[SAL][{self.info_dict["format_id"]}]: HOST[{self._host.split(".")[0]}] ERROR '
                ...
                if (
                    ...
                    if (_eta_smooth := cast(float, _temp.get("eta_smooth"))) and _eta_smooth < 3600:
                        _eta_smooth_str = ":".join(
                            [
                                _item.split(".")[0]
                                for _item in f"{timedelta(seconds=_eta_smooth)}".split(":")[1:]
                            ]
                        )
                    else:
                        _eta_smooth_str = "--"
                    ...
                    msg += (
                        f"DL PR[{self.dl_cont[-1].get('progress_str', '--') if self.dl_cont else '--':>7}]\n"
                    )

        except Exception as e:
            logger.exception(
                f"[{self.info_dict['id']}][{self.info_dict['title']}][print hookup] error {repr(e)}"
            )
