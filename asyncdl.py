...
class AsyncDL:
    ...
    def get_videos_cached(self, *args, **kwargs):
        ...
        logger.info(
            f"[videos_cached] start scanning - dlnocaching [{self.args.nodlcaching}]"
        )
                    ...
                    for _vol, _folder in local_storage.config_folders.items():
                        if _vol != "local":
                            videos_cached.update(
                                local_storage._data_from_file.get(_vol)
                            )
                        else:
                    ...
                    try:
                        ...
                        for file in files:
                            ...
                            try:
                                ...
                                #logger.info(f"[get_videos_cached] {str(file)} {_xattr_desc}")
                        ...
                        last_time_sync.update(
                            {list_folders_to_scan[folder]: str(self.launch_time)}
                        )
            ...
            if _url_list:
                ...
                if netdna_list:
                    ...
                    with ThreadPoolExecutor(
                        thread_name_prefix="Get_netdna",
                        max_workers=min(self.init_nworkers, len(netdna_list)),
                    ) as ex:

                        futures = [
                            ex.submit(_ies_netdna.get_entry, _url_netdna)
                            for _url_netdna in netdna_list
                        ]
                    ...
                    for fut, _url_netdna in zip(futures, netdna_list):
                        try:
                            ...
                            self.list_videos.append(
                                self.info_videos[_url_netdna]["video_info"]
                            )
                        ...
                        while True:
                            ...
                            try:
                                ...
                                if not _info:
                                    try:
                                        _errormsg = None
                                        _info = await self.async_ytdl_extract_info(
                                            _url, download=False, process=False
                                        )
                                    except Exception as e:
                                        logger.warning(
                                            f"[url_playlist_list] {_url} {type(e).__name__}"
                                        )
                                        logger.debug(
                                ...
                                if not _info:
                                    _info = {
                                        "_type": "error",
                                        "url": _url,
                                        "error": _errormsg or "no video entry",
                                    }
                                    ...
                                    if (
                                        ...
                                        _info = self.ytdl.sanitize_info(
                                            await self.async_ytdl_process_ie_result(
                                        ...
                                        for _ent in _info.get("entries"):
                                            ...
                                            if _ent.get(
                                                ...
                                                if (((_ent.get("extractor_key") or _ent.get("extractor") or _ent.get("ie_key")).lower() == "generic") 
    ...
    async def worker_init(self, i):
        ...
        try:
            ...
            while True:
                if self.getlistvid_done.is_set():
                    break
            ...
            while not self.STOP.is_set():
                    ...
                    [
                        ...
                        asyncio.create_task(self.queue_vid.get()),
                    ],
                    ...
                    while True:
                        async with self.alock:
                            if (_val := self.count_init) == (self.init_nworkers - 1):
                                break
                            logger.debug(
                                f"[worker_init][{i}]: bucle while [count_init] {self.count_init} [init_workers] {self.init_workers} [val] {_val}"
                            )
                    ...
                    try:
                        ...
                        if vid.get("_type", "video") != "video":
                            ...
                            try:
                                ...
                                logger.debug(
                                    f"[worker_init][{i}]: [{url_key}] extra_info={_ext_info or vid}"
                                )
                                _res = await self.async_ytdl_extract_info(
                                ...
                                logger.debug(
                                    f"[worker_init][{i}]: [{url_key}] info extracted\n{_for_print(info)}"
                                )
                                ...
                                logger.error(
                                    f"[worker_init][{i}]: [{_pending}/{_to_check}] [{url_key}] init nok - {_error}"
                                )
                        ...
                        async def go_for_dl(urlkey, infdict, extradict=None):
                            ...
                            logger.debug(
                                f"[worker_init][{i}]: [{_pending}/{_to_check}] [{infdict.get('id')}][{infdict.get('title')}] info extracted"
                            )
                            logger.debug(
                                f"[worker_init][{i}]: [{urlkey}] info extracted\n{_for_print(infdict)}"
                            )
                            ...
                            if _path := await self.async_check_if_aldl(infdict):
                                ...
                                logger.debug(
                                    f"[worker_init][{i}][{infdict.get('id')}][{infdict.get('title')}] already DL"
                                )
                        ...
                        async def get_dl(urlkey, infdict, extradict=None):
                            ...
                            if (
                                ...
                                logger.debug(
                                    f"[worker_init][{i}]: [{dl.info_dict['id']}][{dl.info_dict['title']}]: {dl.info_dl}"
                                )
                        ...
                        if (_type := info.get("_type", "video")) == "video":
                            ...
                            logger.warning(
                                f"[worker_init][{i}]: [{url_key}] playlist en worker_init"
                            )
                            ...
                            for _entry in info["entries"]:
                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        logger.warning(
                                            f"[worker_init][{i}]: [{url_key}] playlist of entries that are not videos"
                                        )
                                    else:
                                    ...
                                    logger.error(
                                        f"[worker_init][{i}]: [{_url}] init nok - Error:{repr(e)}"
                                    )
                        ...
                        logger.error(
                            f"[worker_init][{i}]: [{_pending}/{_to_check}] [{url_key}] init nok - Error:{repr(e)}"
                        )
    ...
    async def async_ex(self):
        ...
        self.async_ytdl_extract_info = sync_to_async(
            self.ytdl.extract_info, self.ex_winit
        )
        self.async_ytdl_process_ie_result = sync_to_async(
            self.ytdl.process_ie_result, self.ex_winit
        )
            ...
            if not self.args.nodl:
                ...
                if self.args.aria2c:
                    ...
                    await asyncio.wait(                        [
                            asyncio.create_task(
                                async_ex_in_executor(
                                    self.ex_winit, self.init_aria2c_ready.wait
                                )
                            )
                        ]
                    )
                ...
                done, pending = await asyncio.wait([ _first:=asyncio.create_task(self.getlistvid_first.wait()),
        ...
        for url, video in self.info_videos.items():
            if (
                ...
                if video["status"] == "done":
                    ...
                    _temp["local"].update(
                        {
                            f"{traverse_obj(video, ('video_info', 'id'))}_{traverse_obj(video, ('video_info', 'title')).upper()}": str(
