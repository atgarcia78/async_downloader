...
#     def __init__(self, i, asyncdl):
#         self.i = i
#         self.current_url = None
#         self.status = "init"
#         if not WorkerInit._ASYNCDL:
#             WorkerInit._ASYNCDL = asyncdl

            
#     async def start(self):

        
#         logger.debug(f"[worker_init][{self.i}]: launched")
...
#                 url_key = await WorkerInit._ASYNCDL.queue_vid.async_q.get()

#                 if url_key == "KILL":
#                     logger.debug(f"[worker_init][{self.i}]: finds KILL")
#                     break
#                 elif url_key == "KILLANDCLEAN":
#                     logger.debug(f"[worker_init][{self.i}]: finds KILLANDCLEAN")
                    
#                     while True:
#                         async with WorkerInit._ASYNCDL.alock:
#                             if (_val:=WorkerInit._ASYNCDL.count_init) == (WorkerInit._ASYNCDL.init_nworkers - 1):
#                                 break
#                             logger.debug(f"[worker_init][{self.i}]: bucle while [count_init] {WorkerInit._ASYNCDL.count_init} [init_workers] {WorkerInit._ASYNCDL.init_workers} [val] {_val}")
                        
#                         await asyncio.sleep(5)
                    
#                     WorkerInit._ASYNCDL.t3.stop()                    
                    
#                     for _ in range(WorkerInit._ASYNCDL.workers - 1):
#                         WorkerInit._ASYNCDL.queue_run.put_nowait(("", "KILL"))
                    
#                     WorkerInit._ASYNCDL.queue_run.put_nowait(("", "KILLANDCLEAN"))
                    
#                     if not WorkerInit._ASYNCDL.list_dl: 
#                         #WorkerInit._ASYNCDL.stop_root = True
#                         WorkerInit._ASYNCDL.stop_console = False
#                         WorkerInit._ASYNCDL.pasres_repeat = False
#                         #await asyncio.sleep(0)
...
#                 else: 
#                     async with WorkerInit._ASYNCDL.alock:
#                         _qsize = WorkerInit._ASYNCDL.queue_vid.async_q.qsize() 
#                         if not WorkerInit._ASYNCDL.nowaitforstartdl or WorkerInit._ASYNCDL.getlistvid_done:
#                             _pending = _qsize - WorkerInit._ASYNCDL.init_nworkers - 1
#                         else:
#                             _pending = _qsize 
                    
#                     vid = WorkerInit._ASYNCDL.info_videos[url_key]['video_info']
#                     logger.debug(f"[worker_init][{self.i}]: [{url_key}] extracting info\n{vid}")
                    
#                     try: 
#                         if WorkerInit._ASYNCDL.wkinit_stop:
#                             logger.info(f"[worker_init][{self.i}]: BLOCKED")
                            
#                             while WorkerInit._ASYNCDL.wkinit_stop:
#                                 await async_wait_time(5)
#                                 logger.debug(f"[worker_init][{self.i}]: BLOCKED")
                                
#                             logger.info(f"[worker_init][{self.i}]: UNBLOCKED")
...
#                                 logger.debug(f"[worker_init][{self.i}]: [{url_key}] extra_info={_ext_info or vid}")
#                                 _res = await async_ex_in_executor(WorkerInit._ASYNCDL.ex_winit, WorkerInit._ASYNCDL.ytdl.extract_info, vid['url'], download=False, extra_info=_ext_info)
#                                 if not _res: raise Exception("no info video")
#                                 info = WorkerInit._ASYNCDL.ytdl.sanitize_info(_res)
#                                 logger.debug(f"[worker_init][{self.i}]: [{url_key}] info extracted\n{info}")
                            
#                             except Exception as e: 
                                
#                                 if 'unsupported url' in str(e).lower():                                    
#                                     WorkerInit._ASYNCDL.list_unsup_urls.append(url_key)
...
#                                     WorkerInit._ASYNCDL.list_notvalid_urls.append(url_key)                                    
                                    
#                                 else: 
#                                     _error = repr(e)
#                                     WorkerInit._ASYNCDL.list_urls_to_check.append((url_key, _error))
                                   
                                
#                                 WorkerInit._ASYNCDL.list_initnok.append((url_key, _error))
#                                 WorkerInit._ASYNCDL.info_videos[url_key]['error'].append(_error)
#                                 WorkerInit._ASYNCDL.info_videos[url_key]['status'] = 'initnok'
                                
#                                 logger.error(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{url_key}] init nok - {_error}")                                
...
#                             logger.debug(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{infdict.get('id')}][{infdict.get('title')}] info extracted")                        
#                             logger.debug(f"[worker_init][{self.i}]: [{urlkey}] info extracted\n{infdict}")
                            
#                             WorkerInit._ASYNCDL.info_videos[urlkey].update({'video_info': infdict})
                            
                            
#                             _filesize = none_to_cero(extradict.get('filesize', 0)) if extradict else none_to_cero(infdict.get('filesize', 0))
                            
#                             if (_path:=WorkerInit._ASYNCDL._check_if_aldl(infdict)):
                                
#                                 logger.info(f"[worker_init][{self.i}]: [{infdict.get('id')}][{infdict.get('title')}] already DL")                               
                                
                                
#                                 if _filesize:
#                                     async with WorkerInit._ASYNCDL.alock:
#                                         WorkerInit._ASYNCDL.totalbytes2dl -= _filesize
                                    
#                                 WorkerInit._ASYNCDL.info_videos[urlkey].update({'status': 'done', 'aldl': _path})                                        
#                                 return False
                            
#                             if (_same_video_url:=WorkerInit._ASYNCDL._check_if_same_video(urlkey)):
                                                                
#                                 if _filesize:
#                                     async with WorkerInit._ASYNCDL.alock:
#                                         WorkerInit._ASYNCDL.totalbytes2dl -= _filesize
                                
#                                 WorkerInit._ASYNCDL.info_videos[urlkey].update({'samevideo': _same_video_url})
...
#                                 if (await go_for_dl(urlkey ,infdict, extradict)) and not WorkerInit._ASYNCDL.args.nodl:
                                    
                                                                        
#                                     dl = await async_ex_in_executor(WorkerInit._ASYNCDL.ex_winit, VideoDownloader, WorkerInit._ASYNCDL.info_videos[urlkey]['video_info'], WorkerInit._ASYNCDL.ytdl, WorkerInit._ASYNCDL.args)
                                    
#                                     logger.debug(f"[worker_init][{self.i}]: [{dl.info_dict['id']}][{dl.info_dict['title']}]: {dl.info_dl}")
...
#                                             WorkerInit._ASYNCDL.info_videos[urlkey]['video_info']['filesize'] = dl.info_dl.get('filesize')
#                                             async with WorkerInit._ASYNCDL.alock:
#                                                 WorkerInit._ASYNCDL.totalbytes2dl = WorkerInit._ASYNCDL.totalbytes2dl - _filesize + dl.info_dl.get('filesize', 0)
                                                
#                                         WorkerInit._ASYNCDL.info_videos[urlkey].update({'status': 'initok', 'filename': dl.info_dl.get('filename'), 'dl': dl})

#                                         async with WorkerInit._ASYNCDL.alock:
#                                             WorkerInit._ASYNCDL.list_dl.append(dl)
                                            
                                        
#                                         if dl.info_dl['status'] in ("init_manipulating", "done"):
#                                             WorkerInit._ASYNCDL.queue_manip.put_nowait((urlkey, dl))
#                                             logger.info(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{dl.info_dict['id']}][{dl.info_dict['title']}]: init OK, video parts DL")
                                        
#                                         else:
#                                             logger.debug(f"[worker_init][{self.i}]: [{dl.info_dict['id']}][{dl.info_dict['title']}]: init OK, lets put in run queue")
#                                             WorkerInit._ASYNCDL.queue_run.put_nowait((urlkey, dl))
#                                             _msg = ''
#                                             async with WorkerInit._ASYNCDL.alock:
#                                                 if dl.info_dl.get('auto_pasres'):
#                                                     _index_in_dl = len(WorkerInit._ASYNCDL.list_dl)
#                                                     WorkerInit._ASYNCDL.list_pasres.add(_index_in_dl)
#                                                     _msg = f', add this dl[{_index_in_dl}] to auto_pasres{list(WorkerInit._ASYNCDL.list_pasres)}'
#                                                     if WorkerInit._ASYNCDL.window_console and not WorkerInit._ASYNCDL.stop_root: sg.cprint(f"[pause-resume autom] {WorkerInit._ASYNCDL.list_pasres}")
                                                                                
#                                             logger.info(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{dl.info_dict['id']}][{dl.info_dict['title']}]: init OK, ready to DL{_msg}")
...
#                             WorkerInit._ASYNCDL.info_videos[url_key]['todl'] = False
...
#                                         logger.warning(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{url_key}] playlist of entries that are not videos")
#                                     else:
#                                         _url = _entry.get('original_url') or _entry.get('url')
                        
#                                         if not WorkerInit._ASYNCDL.info_videos.get(_url): #es decir, los nuevos videos 
                            
#                                             WorkerInit._ASYNCDL.info_videos[_url] = {'source' : 'playlist', 
#                                                                         'video_info': _entry, 
#                                                                         'status': 'init', 
#                                                                         'aldl': False,
#                                                                         'todl': True,
...
#                                             _same_video_url = WorkerInit._ASYNCDL._check_if_same_video(_url)
                            
#                                             if _same_video_url: 
                                
#                                                 WorkerInit._ASYNCDL.info_videos[_url].update({'samevideo': _same_video_url})
#                                                 logger.warning(f"{_url}: has not been added to video list because it gets same video than {_same_video_url}")
                                        
#                                             else:
#                                                 async with WorkerInit._ASYNCDL.alock:
#                                                     WorkerInit._ASYNCDL.totalbytes2dl += none_to_cero(_entry.get('filesize', 0))
#                                                 if WorkerInit._ASYNCDL.wkinit_stop:
#                                                     logger.info(f"[worker_init][{self.i}]: BLOCKED")
...
#                                     WorkerInit._ASYNCDL.list_initnok.append((_entry, f"Error:{repr(e)}"))
#                                     logger.error(f"[worker_init][{self.i}]: [{_url}] init nok - Error:{repr(e)} \n{'!!'.join(lines)}")
                                    
#                                     WorkerInit._ASYNCDL.list_urls_to_check.append((_url,repr(e)))
#                                     WorkerInit._ASYNCDL.info_videos[_url]['error'].append(f'DL constructor error:{repr(e)}')
#                                     WorkerInit._ASYNCDL.info_videos[_url]['status'] = 'initnok'                                    
                                    
#                                     continue

#                     except Exception as e:
#                         lines = traceback.format_exception(*sys.exc_info())
#                         WorkerInit._ASYNCDL.list_initnok.append((vid, f"Error:{repr(e)}"))
#                         logger.error(f"[worker_init][{self.i}]: [{WorkerInit._ASYNCDL.num_videos_to_check - _pending}/{WorkerInit._ASYNCDL.num_videos_to_check}] [{url_key}] init nok - Error:{repr(e)} \n{'!!'.join(lines)}")
                        
#                         WorkerInit._ASYNCDL.list_urls_to_check.append((url_key,repr(e)))
#                         WorkerInit._ASYNCDL.info_videos[url_key]['error'].append(f'DL constructor error:{repr(e)}')
#                         WorkerInit._ASYNCDL.info_videos[url_key]['status'] = 'initnok'
...
#         except Exception as e:
#             lines = traceback.format_exception(*sys.exc_info())
#             logger.error(f"[worker_init][{self.i}]: Error:{repr(e)} \n{'!!'.join(lines)}")
                    
#         finally:
#             async with WorkerInit._ASYNCDL.alock:
#                 WorkerInit._ASYNCDL.count_init += 1                
#             logger.debug(f"[worker_init][{self.i}]: BYE")
#             await asyncio.sleep(0)
...
class AsyncDL():
    ...
    def get_videos_cached(self, nodlcaching, queue):
    ...
    def _prepare_for_dl(self, url):
        ...
        if self.info_videos[url].get('todl') and not self.info_videos[url].get('aldl') and not self.info_videos[url].get('samevideo') and self.info_videos[url].get('status') != 'prenok':
            ...
            self.queue_vid.sync_q.put_nowait(url)
                ...
                if url_key == "KILL":
                    ...
                    async with self.alock:
                        ...
                        if not self.nowaitforstartdl or self.getlistvid_done:
                            _pending = _qsize - self.init_nworkers - 1
                        else:
                            _pending = _qsize 
    ...
    def get_results_info(self):
        ...
        # def _getter(url, vid):
        #     webpageurl = vid['video_info'].get('webpage_url')
        #     originalurl = vid['video_info'].get('original_url')
        #     playlist = vid['video_info'].get('playlist')            
        #     if url == webpageurl:
