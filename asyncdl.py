...
class Workers:
    def __init__(self, asyncdl, name):
        self.asyncdl = asyncdl
        self.logger = logging.getLogger(name)
        self._max = asyncdl.max_workers
        self.running = deque()
        self.waiting = deque()
        self.tasks = {}
        self.info_dl = {}
...
class WorkersRun(Workers):
    def __init__(self, asyncdl):
        super().__init__(asyncdl, asyncdl.workers, "WorkersRun")
...
class WorkersInit(Workers):
    def __init__(self, asyncdl):
        super().__init__(asyncdl, asyncdl.init_nworkers, "WorkersInit")
...
class AsyncDL:
    ...
    async def get_dl(self, url_key):
        ...
        async with self.alock:
            ...
            self.info_videos[url_key] |= {"status": "initok",
    ...
    async def init_callback(self, url_key):
        ...
        try:
            ...
            if (_type := info.get("_type", "video")) == "video":
                ...
                #                 else:
                #                     try:
                #                         if not self.STOP.is_set() and await self._prepare_for_dl(_url, put=False):
                #                             self.list_videos.append(self.info_videos[_url]["video_info"])
