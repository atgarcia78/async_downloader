...
class AsyncHTTPDownloader():
        ...
        while not self.parts_queue.empty():
            ...
            try:
                ...
                async with self.client.stream("GET", self.video_url, 
                    headers=self.parts_header[part-1]) as res:            
                    
                    if res.status_code >= 400:                               
                        self.parts_queue.put_nowait(part)
                        self.logger.debug(f"{self.webpage_url}: Reinserted Part_{part} resp code:{str(res)}")
    ...
    async def fetch_async(self):
        ...
        for part in range(1, self.n_parts+1):
            ...
            self.logger.debug(f"{tempfilename}:{tempfilesize}")
            if not tempfilename.exists():
                self.logger.debug(f"{self.webpage_url}: Part_{part} don't exits, lets DL")
                self.parts_queue.put_nowait(part)
            else:
                partsize = tempfilename.stat().st_size
                if partsize in range(tempfilesize - 100, tempfilesize + 100):
                    self.logger.debug(f"{self.webpage_url}: Part_{part} exits with size {partsize} and full downloaded")
                    self.down_size += partsize
                    continue
                else:
                    self.logger.debug(f"{self.webpage_url}: Part_{part} exits with size {partsize} and not full downloaded {tempfilesize}. Reload")
                    tempfilename.unlink()
                    self.parts_queue.put_nowait(part)
        ...
        try:
            ...
            #     await loop.run_in_executor(pool, self.ensamble_file())
            # self.logger.debug(f"{self.webpage_url}: out of the coroutine in other thread")
            #result.result()
            self.ensamble_file()
            self.logger.debug(f"{self.webpage_url}: ensambled OK")
        except Exception as e:
            self.logger.warning(f"{self.webpage_url}: error when ensambling parts {e} {sys.exc_info()}")
        ...
        if len(part_files) != self.n_parts:
            ...
            for i, f in enumerate(part_files):
                ...
                if partsize not in range(tempfilesize - 100, tempfilesize + 100):
                    ...
                    self.status = "error"
                    raise AsyncHTTPDLError(f"{self.webpage_url}:part {i+1} file size {partsize} dont expected {tempfilesize}")
                else:
