...
class AsyncFFmpegFD(FFmpegFD):
    ...
    async def _async_call_downloader(self, tmpfilename, info_dict, queue):
        ...
        try:
            ...
            if (
                "duration" in info_dict.keys()
                and info_dict["duration"] is not None
            ...
            async def read_stream(stream):
                try:
                    ...
                    while not proc.returncode:
                        ...
                        if line:
                            ...
                            if ffmpeg_prog_infos:
                                ...
                                speed = (
                                    0
                                    if ffmpeg_prog_infos["speed"] == "N/A"
                                    else float(ffmpeg_prog_infos["speed"][:-1])
                                )
                                if speed != 0:
                                    eta_seconds = (
                                        total_time_to_dl
                                        - parse_ffmpeg_time_string(ffmpeg_prog_infos["out_time"])
                                    ) / speed
                                else:
                                    eta_seconds = 0
                                bitrate_int = None
                                bitrate_str = re.match(
                                    r"(?P<E>\d+)(\.(?P<f>\d+))?(?P<U>g|m|k)?bits/s",
                    ...
                    status.update({"status": "finished", "downloaded_bytes": total_filesize})
                    logger.debug(f"[{info_dict['id']}][{info_dict['title']}]\n{status}")
                    queue.put_nowait(status)
...
class AsyncFFMPEGDownloader:
    ...
    async def fetch_async(self):
        ...
        try:
            task_dl = asyncio.create_task(
                self.ffmpegfd._async_call_downloader(str(self.filename), self.info_dict, self.queue)
            )
