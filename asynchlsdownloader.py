...
class AsyncHLSDownloader:
    ...
    def __init__(self, enproxy: bool, video_dict: dict, vid_dl):
        ...
        try:
            ...
            self.filesize = None
            self.premsg = f"[{self.info_dict['id']}]\
        ...
        def getter(x: Union[str, None]):
            try:
                ...
                if "nakedsword" in x:
                    ...
                    logger.debug(
                        f"{self.premsg} \
{_for_print_entry(self.info_dict)}")
                    ...
                    if self.fromplns:
                        ...
                        logger.debug(
                            f"{self.premsg}: \
added new dl to plns [{self.fromplns}], \
count [{len(_downloaders)}] \
members[{list(_downloaders.keys())}]"
                        )
            except Exception as e:
                logger.exception(
                    f"{self.premsg}: {str(e)}"
...
{self.key_cache[_frag.key.absolute_uri]}")
...
{repr(e)} {_for_print(info_reset)}"
    ...
    def resetdl(self, cause=None):
        ...
        logger.info(f"{self.premsg}:RESET[{self.n_reset}] cause[{cause}] \
fromplns[{self.fromplns}]")
        ...
        try:
            ...
            if cause and str(cause) == "403":
                ...
                logger.info(f"{self.premsg}:RESET[{self.n_reset}] \
start wait in reset cause 403")
                if not wait_time(CONF_HLS_RESET_403_TIME,
                                 event=self.vid_dl.stop_event):
                    return

                logger.info(f"{self.premsg}:RESET[{self.n_reset}] \
fin wait in reset cause 403")
            ...
            if self.fromplns and cause in ("403", "hard"):
                ...
                with (_sem := self.vid_dl.info_dl[
                    ...
                    with (_sem2 := self.vid_dl.info_dl[
                            "fromplns"][self.fromplns]["sem"]):
...
in sem2")
            ...
            logger.exception(f"{self.premsg}:RESET[{self.n_reset}]: \
outer Exception occurred when reset: {repr(e)}")
            raise
        finally:
            if self.fromplns and cause in ("403", "hard"):

                _inreset = self.vid_dl.info_dl["fromplns"][
                        self.fromplns]["in_reset"]
                try:
...
from {_inreset}"
                ...
                if not self.vid_dl.info_dl["fromplns"][
                    ...
                    logger.info(f"{self.premsg}:RESET[{self.n_reset}]: \
end of resets fromplns [{self.fromplns}]")

                    self.vid_dl.info_dl["fromplns"][
                        self.fromplns]["reset"].set()
                    self.vid_dl.info_dl["fromplns"][
                        self.fromplns]["sem"] = threading.BoundedSemaphore(
                        value=1)

                    self.vid_dl.info_dl["fromplns"]["ALL"][
                        "in_reset"].remove(self.fromplns)
                    if not self.vid_dl.info_dl["fromplns"]["ALL"][
                            "in_reset"]:
                        self.vid_dl.info_dl["fromplns"]["ALL"][
                            "reset"].set()
                        self.vid_dl.info_dl["fromplns"]["ALL"][
                    ...
                    logger.info(
                        f"{self.premsg}:RESET[{self.n_reset}]: \
waits for rest scenes to start DL [{self.fromplns}]"
                    )
...
\n\ninfo_frag {len(self.info_frag)}"
        ...
        if not self.frags_to_dl:
            ...
            logger.debug(f"{self.premsg}:RESET[{self.n_reset}]:prep_reset:OK \
{self.frags_to_dl[0]} .. {self.frags_to_dl[-1]}")
...
decrease speed: throttle[{_old_throttle} -> \
{self.throttle}] n_el_speed[{len(_speed)}]"
                        )
                        logger.debug(
                            f"{self.premsg}[check_speed] MAX BD -> \
decrease speed: throttle[{_old_throttle} -> \
...
%no%{_str_speed}"
                        ...
                        self._test.append((
                            f"estable throttle {self.throttle}"))
...
increase speed: throttle[{_old_throttle} -> \
{self.throttle}] n_el_speed[{len(_speed)}]"
                        )
                        logger.debug(
                            f"{self.premsg}[check_speed] MIN 0.8 -> \
...
{self.throttle}]"
            ...
            logger.warning(
                f"{self.premsg}[check_speed] {repr(e)}"
            )
        finally:
    ...
    async def upt_status(self):
        ...
        while not any([self.kill.is_set(),
                       self.vid_dl.end_tasks.is_set()]):
            ...
            if self.progress_timer.has_elapsed(seconds=CONF_INTERVAL_GUI / 2):
                ...
                if self.first_data.is_set() and not any(
                    ...
                    self._test.append((time.monotonic(),
                                       _down_size,
                                       _speed_meter))
                    ...
                    if _speed_meter and self.filesize:
                        ...
                        self._speed.append((datetime.now(),
                                            copy.deepcopy(self.upt)))
                        # if self._qspeed:
        ...
        try:
            ...
            while True:
                ...
                if _res.get("event"):
                    ...
                    raise AsyncHLSDLError(f"couldnt get frag from queue \
{repr(_e)}")
                else:
                    q = _res.get("result")
                    if q is None:
                ...
                logger.debug(f"{self.premsg}:[worker-{nco}]: frag[{q}]\n\
{self.info_frag[q - 1]}")
                ...
                while self.info_frag[q - 1][
                    ...
                    try:
                        ...
                        self._premsg = f"{self.premsg}:[worker-{nco}]:\
[frag-{q}] "

                        async with self._limit:
                            logger.debug(f"{self._premsg}: \
limiter speed")
                        ...
                        async with aiofiles.open(
                            ...
                            logger.debug(
                                f"{self._premsg}: \
{res.request} {res.status_code} \
{res.reason_phrase} {res.headers}")
                            ...
                            if res.status_code == 403:
                                if self.fromplns:
                                    ...
                                    _wait_tasks = await self.vid_dl.reset(
                                        "403")
                                logger.debug(
                                    f"{self._premsg}: \
wait_tasks\n{_wait_tasks}\n\
...
{_size}]"
                                        ...
                                        logger.warning(
                                            f"{self.premsg}: frag with\
mark downloaded but file \
[{filename}] doesnt exists"
                                ...
                                if self.info_frag[
                                    q - 1][
                                    "headersize"] and self.info_frag[
                                    q - 1][
                                        "headersize"] < self._CHUNK_SIZE:

                                    _chunk_size = self.info_frag[
                                        q - 1][
                                        "headersize"
                                    ]
...
{self.info_frag[q - 1]}"
                            ...
                            logger.warning(
                                f"{self._premsg}: end of streaming. \
fragment not completed\n\
{self.info_frag[q - 1]}"
                            )
                            raise AsyncHLSDLError(
                        ...
                        logger.debug(f"{self._premsg}: errorfatal \
{repr(e)}")
                        ...
                        if self.info_frag[q - 1][
                            ...
                            self.info_frag[q - 1]["error"].append(
                                "MaxLimitRetries")
                            logger.warning(f"{self._premsg}: \
MaxLimitRetries:skip")
            ...
            async with self._LOCK:
                self.count -= 1
            logger.debug(
                f"{self.premsg}:[worker{nco}]: bye worker"
            )
            await client.aclose()
    ...
    async def fetch_async(self):
        ...
        if self.fromplns:
            _event = traverse_obj(
                self.vid_dl.info_dl["fromplns"], ("ALL", "reset")
            )
            if _event:
                    ...
                    if n_frags_dl == len(self.info_dict["fragments"]):
                        ...
                        if self.vid_dl.stop_event.is_set():
                            ...
                            if _cause := self.vid_dl.reset_event.is_set():
                                ...
                                dump_init_task = [asyncio.create_task(
                                    sync_to_async(self.dump_init_file,
                                                  executor=self.ex_dl)())]
                                ...
                                if self.n_reset < self._MAX_RESETS:
                                    ...
                                    if self.fromplns and _cause in (
                                            "403", "hard"):
...
{len(self.fragsnotdl())}"
                                        ...
                                        logger.exception(
                                            f"{self.premsg}:RESET\
[{self.n_reset}]:ERROR reset \
couldnt progress:[{repr(e)}]"
                                        )
                                        self.status = "error"
                                        await self.clean_when_error()
                                        raise AsyncHLSDLErrorFatal(
                                            f"{self.premsg}: ERROR reset \
couldnt progress"
                                        )
                                    ...
                                    logger.warning(
                                        f"{self.premsg}:RESET[{self.n_reset}]:\
ERROR:Max_number_of_resets"
                                    )
                                    self.status = "error"
                                    await self.clean_when_error()
                                    await asyncio.sleep(0)
                                    raise AsyncHLSDLErrorFatal(
                                        f"{self.premsg}: ERROR max resets"
                                    )
...
{inc_frags_dl}] new cycle with no \
fatal error"
                                    )
                                    try:
                                        await self.areset()
    ...
    async def clean_from_reset(self):
        ...
        try:
            ...
            if not self.vid_dl.info_dl["fromplns"][
                ...
                logger.info(
                    f"{self.premsg} end of resets fromplns \
[{self.fromplns}]"
                )
                try:
                    self.vid_dl.info_dl["fromplns"]["ALL"][
                    ...
                    logger.warning(
                        f"{self.premsg} error when removing \
[{self.fromplns}] from \
{self.vid_dl.info_dl['fromplns']['ALL']['in_reset']}"
                    )
            ...
            logger.warning(
                f"{self.premsg} error when removing \
[{self.vid_dl.info_dict['playlist_index']}] from\
{self.vid_dl.info_dl['fromplns'][self.fromplns]['in_reset']}")
    ...
    def ensamble_file(self):
        ...
        try:
            logger.debug(f"{self.premsg}:{self.filename}")
            ...
            if self.filename.exists():
                ...
                if _skipped:
                    logger.warning(
                        f"{self.premsg}: [ensamble_file] skipped frags \
[{_skipped}]"
                    )
            else:
                self.status = "error"
    ...
    def print_hookup(self):
        ...
        if self.status == "done":
            return f"[HLS][{self.info_dict['format_id']}]: PROXY[{_proxy}] \
Completed \n"
        elif self.status == "init":
            return f"[HLS][{self.info_dict['format_id']}]: PROXY[{_proxy}] \
Waiting to DL [{_filesize_str}] \
[{self.n_dl_fragments:{format_frags()}}/{self.n_total_fragments}]\n"
...
[{self.n_dl_fragments:{format_frags()}}/{self.n_total_fragments}]\n"
            ...
            return f"[HLS][{self.info_dict['format_id']}]: PROXY[{_proxy}] \
WK[{self.count:2d}/{self.n_workers:2d}] \
FR[{self.n_dl_fragments:{format_frags()}}/{self.n_total_fragments}\
] PR[{_progress_str}] DL[{_speed_meter_str}] ETA[{_eta_smooth_str}\
]"

        elif self.status == "init_manipulating":
            return f"[HLS][{self.info_dict['format_id']}]: \
